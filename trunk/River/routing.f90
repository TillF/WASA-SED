...
INTEGER :: itl, itr, ih, i, j, istate, h, hrout_t, hrout_s !, mm, imunout, iout, make
...
! (given parameter lag-time tL and retention-time tR)
  hrout_t=0
  hrout_s=0
  DO i=1,size(prout,dim=1)
    hrout_t = nint(prout(i,1)) + nint(prout(i,2))
    if (hrout_t > hrout_s) hrout_s = hrout_t
  END DO

  allocate( hrout(hrout_s,subasin)) !allocate memory for triangular unit hydrograph
  hrout(:,:)=0.
